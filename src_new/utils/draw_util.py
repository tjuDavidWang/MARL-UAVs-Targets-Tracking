...
def update(frame, env, uav_plots, target_plots, uav_search_patches, steps, trace, internal):
    for i, uav in enumerate(env.uav_list):
        uav_x = env.position['all_uav_xs'][max(0, frame * steps - trace * internal): frame * steps + 1: internal]
        uav_x = [sublist[i] for sublist in uav_x]
        uav_y = env.position['all_uav_ys'][max(0, frame * steps - trace * internal): frame * steps + 1: internal]
    ...
    for i in range(env.m_targets):
        target_x = env.position['all_target_xs'][max(0, frame * steps - trace * internal): frame * steps + 1: internal]
        target_x = [sublist[i] for sublist in target_x]
        target_y = env.position['all_target_ys'][max(0, frame * steps - trace * internal): frame * steps + 1: internal]
...
def draw_animation(config, env, num_steps, ep_num, frames=100, trace=10, internal=10):  # num_steps % frames must be 0
    ...
    target_plots = [ax.plot([], [], marker='o', color='r', linestyle='None')[0] for _ in range(env.m_targets)]

    uav_search_patches = [patches.Circle((uav.x, uav.y), uav.dp, color='lightblue', alpha=0.2)
